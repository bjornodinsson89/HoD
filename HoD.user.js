...
// @updateURL    https://github.com/BjornOdinsson89/HoD/raw/main/HoD.meta.js
...
.attack-btn { background: #4CAF50; color: white; border: none; padding: 2px 4px; cursor: pointer; transition: transform 0.2s; border-radius: 3px; font-size: 12px; }
.attack-btn:hover { transform: scale(1.1); }
.attack-btn:active { transform: scale(0.95); }
.attack-btn.disabled { background: gray; cursor: not-allowed; }
`);
    ...
    class HoDOverlay extends BaseModule {
        ...
        async startMembersPoll(sectionContent) {
        ...
        async startEnemyPoll(sectionContent) {
            ...
            const interval = setInterval(() => {
                ...
                countdowns.forEach(span => {
                    ...
                    if (timer > 0) {
                        ...
                        if (td) {
                            td.innerHTML = 'Okay';
                            // Re-attach event if needed, but since re-render, handled in attach
                        }
                    }
                });
            }, 1000);
        ...
        parseLastAction(str) {
            if (str === 'Online' || str === 'Just now') return 0;
